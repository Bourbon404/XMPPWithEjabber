...
//    NSLog(@"presence = %@", presence);
    ...
    if (![presenceFromUser isEqualToString:userId]) {
        ...
        if ([presenceType isEqualToString:@"available"]) {
            ...
            //用户列表委托(后面讲)
            [chatDelegate newBuddyOnline:[NSString stringWithFormat:@"%@@%@", presenceFromUser, kServer]];
            
        }else if ([presenceType isEqualToString:@"unavailable"]) {
            //用户列表委托(后面讲)
            [chatDelegate buddyWentOffline:[NSString stringWithFormat:@"%@@%@", presenceFromUser, kServer]];
        }
    }
